	...
	parser.on('data', function(data) {
		...
		try {
			...
			if (obj.frame == 'data') {
				...
				myHeater.set({"hours" : obj.runtime });
				myHeater.set({"burnerStarts" : obj.starts});
			} 
			else if(obj.frame == 'statusdata') {
				
				myHeater.set({"voltageBattery" : obj.Spannung });
				myHeater.set({"temp" : obj.PT1000_1});
				myHeater.set({"burnerFault" : obj.BrennerStoerung});
