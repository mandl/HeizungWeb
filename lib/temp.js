...
try {
	...
	var db3 = rrdtool.open(f3);
        ...
        var f5 = path.join(__dirname, 'weather4.rrd');
        var db5 = rrdtool.open(f5);
...
var ResetArduino = function (port) {

	const serialAr = new SerialPort(port, { baudRate: 1200 }, () => {
		console.log('Port Opened')
		serialAr.set({ dtr: false });
		serialAr.close()
...
var updateDB4 = function (dataTemp) {

        logger.debug('updateDB4');
        logger.debug(dataTemp);

        try {
                db5.update(dataTemp, function (err) {
                        if (err != null)
                                logger.error(err);
                });
        } catch (e) {
	...
	setInterval(function () {
		logger.debug('checkstation list');
...
exports.updateDB4 = updateDB4;
exports.getJSONRddData = getJSONRddData;
exports.ResetArduino = ResetArduino
